...
const Voted = () => {
	...
	return (
		<div className="bg-void h-screen w-full relative">
					...
					<div className="flex-1 z-20">
						{isShowingVotes && votes ? (
								...
								<ul className="flex flex-col self-center space-y-2 mt-8 w-2/3">
									{votes.map((vote) => (
										<li key={vote.id} className="flex items-center justify-between tracking-wider uppercase text-sm">
											<div className="flex space-x-2 items-center text-ink">
												...
												<b className="text-white">{vote.participant.name}</b>
											</div>
											<span className="text-white">{vote.category.name}</span>
										</li>
									))}
								</ul>
							</div>
							...
							<div>
								...
								<p className="font-light leading-tight mt-3 text-ink">We've sent your votes, now it's time to <br /> wait for the final results.</p>
